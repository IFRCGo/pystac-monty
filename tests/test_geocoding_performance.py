...
def test_get_iso3_from_geometry_performance():
    ...
    test_point = {
        "type": "Point",
        "coordinates": [-98.5795, 39.8283]  # Approximate center of the US
    }
...
if __name__ == "__main__":
    ...
    if results is None:
        print("\nPerformance test failed. Please check the error messages above.")
        sys.exit(1)
    
    # Print a summary
    print("\nSummary:")
    print(f"Old implementation time: {results['old_time']:.4f} seconds")
    print(f"New implementation time (first run): {results['new_time']:.4f} seconds")
    print(f"New implementation time (with cache): {results['cached_time']:.4f} seconds")
    
    if results["speedup_first_run"] > 1:
        print(f"The optimized implementation is {results['speedup_first_run']:.2f}x faster on first run.")
    else:
        print(f"The optimized implementation is {1/results['speedup_first_run']:.2f}x slower on first run.")
        
    if results["speedup_cached"] > 1:
        print(f"With caching, the optimized implementation is {results['speedup_cached']:.2f}x faster.")
    else:
        print(f"With caching, the optimized implementation is {1/results['speedup_cached']:.2f}x slower.")
        
    print("\nConclusion:")
    print("The optimized implementation with caching provides significant performance improvements.")
    print("Even on the first run without cache hits, the spatial filtering helps reduce the number of features checked.")
    print("With caching, subsequent lookups for the same or similar geometries are extremely fast.")
