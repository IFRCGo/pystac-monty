...
class Pairing:
    ...
    def generate_correlation_id(self, item: Item, hazard_profiles: HazardProfiles) -> str:
        ...
        event_id = f"{eventdatestr}-{country_codes[0]}-{block_id}-{hazard_cluster_code}-GCDB"  # noqa: E501
        return event_id

    def generate_guid(self, source_name: str, event_id: str, item: Item, hazard_profiles: HazardProfiles) -> str:
        ...
        hazard_cluster_code = hazard_profiles.get_canonical_hazard_codes(item)[0].upper()
        # This should be dynamically determined based on existing events
        eventdatestr = event_datetime.strftime("%Y%m%dT%H:%M:%S")
        ...
        event_id = (
            f"{eventdatestr}-{country_codes[0]}-{block_id}-{hazard_cluster_code}-GCDB-{source_name}-{event_id}-{episode_number}"  # noqa: E501
        )
        return event_id
