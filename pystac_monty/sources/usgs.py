...
class USGSTransformer(MontyDataTransformer[USGSDataSource]):
    ...
    def _get_primary_country_from_losses(self, losspager_items: typing.List[EmpiricalValidator] | None) -> typing.Optional[str]:
        ...
        return self.geocoder.get_iso3_from_iso2(primary_iso2)
    ...
    def _create_impact_item_from_losses(
        ...
        impact_item = event_item.clone()
        impact_item.id = (
            f"{impact_item.id.replace(STAC_EVENT_ID_PREFIX, STAC_IMPACT_ID_PREFIX)}"
            f"-{impact_type}-{self.geocoder.get_iso3_from_iso2(iso2)}"
        )
        ...
        # Add impact detail
        monty = MontyExtension.ext(impact_item)
        monty.country_codes = [self.geocoder.get_iso3_from_iso2(iso2)]
        monty.impact_detail = ImpactDetail(
            category=category,
            type=imp_type,
            value=value,
