    ...
    def make_calibrated_dataset_response_items(
        ...
        for dataset in datasets:
            ...
            if raw_datetime is None:
                # Fall back to the activation event datetime when the dataset carries none.
                raw_datetime = event_item.properties.get("datetime") or event_item.datetime
                if raw_datetime is None:
                    logger.warning("Skipping calibrated dataset %s: missing datetime", source_id)
                    continue
                logger.warning("Calibrated dataset %s has no datetime, using the event datetime", source_id)
